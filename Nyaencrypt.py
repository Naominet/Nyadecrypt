"""Nyaencrypt.py - single-file CrackProof packer / re-packer.

Auto-merged from pack.py + crackproof_repack.py +
crackproof_encrypt_primitives.py + crackproof_lz.py. The only remaining
dependency is the sibling unpacker Nyadecrypt.py (with legacy decrypt.py fallback) (loaded as
`d`), which is reused to parse the runtime profile before re-packing, plus the
bundled native-EXE and managed-DLL runtime profiles. AES tables are generated in code when the aes_*
files / aes-dir are absent.

Usage:
  python Nyaencrypt.py <unpacked.exe>              # pack (default)
  python Nyaencrypt.py <unpacked.exe> -o out.exe
  python Nyaencrypt.py extract <packed.exe>        # save stub profile
"""
import argparse
...
_MODULE_CANDIDATES = (SCRIPT_DIR / 'Nyadecrypt.py', SCRIPT_DIR / 'decrypt.py')
MODULE_PATH = next((path for path in _MODULE_CANDIDATES if path.is_file()), None)
DEFAULT_AES_DIR = None
if MODULE_PATH is None:
    names = ', '.join(path.name for path in _MODULE_CANDIDATES)
    raise FileNotFoundError(f'unpacker module not found next to this script (tried: {names})')
_spec = importlib.util.spec_from_file_location('Nyadecrypt_mod', MODULE_PATH)
if _spec is None or _spec.loader is None:
    raise ImportError(f'cannot load unpacker module: {MODULE_PATH}')
...
def apply_data8_page(buf, page_off, key):
    ...
    Mirrors Nyadecrypt _apply_d8_page exactly."""
    k = key & M
    k = ((k >> 15) | (k << 17)) & M
...
# import time would fail because Nyadecrypt loads its tables only when
...
def _ensure_sbox():
    ...
    if d._sbox is None:
        raise RuntimeError(
            'AES tables not loaded; call Nyadecrypt.load_aes_tables(dir) first')
...
def aes_encrypt_cbc(buf, data_off, size, key_off):
    """Inverse of Nyadecrypt.aes_decrypt. CBC, IV=0, key schedule is the
...
def aes_encrypt_cbc_ek(buf, data_off, size, ek):
    ...
    matching Nyadecrypt.aes_decrypt (nblocks = size >> 4)."""
    prev = bytes(16)
    for bi in range(size >> 4):
        off = data_off + bi * 16
        plain = bytes(buf[off:off + 16])
...
d = e.d  # the Nyadecrypt module (shared singleton)
...
def decrypt_shell_plaintext(file_data):
    ...
    IMPORTANT: unlike Nyadecrypt.main() Stage 2, we do NOT apply the
...
def unpack_template_pe32p(file_data):
    ...
    Mirrors Nyadecrypt.main() PE32+ path through the file-decode loop."""
    info = d.decrypt_data1(file_data)
    if info[1] != 0x4E4E4F4B:
        raise ValueError('not a CrackProof sample (bad KONN magic)')
    pe = u32(file_data, 0x3C)
...
def unpack_template_pe32(file_data):
    ...
    TemplateProfile. Mirrors Nyadecrypt.main() PE32 path through the
...
def _verify_roundtrip(repacked_bytes, reference_unpack, aes_dir, tag, prof=None,
                      extra_allowed=None):
    """Write the repacked file, run Nyadecrypt on it, and compare the
...
# Bundled canonical runtime profiles.  They are format/runtime profiles, not a
# universal header: profile class must match the target (native EXE, managed DLL,
# etc.).  AES tables are generated in code.
BUNDLED_PROFILE = Path(__file__).resolve().parent / 'stub_profile.bin'
BUNDLED_DOTNET_DLL_PROFILE = (Path(__file__).resolve().parent /
                              'stub_profile_dotnet_dll.bin')
BUNDLED_NATIVE_DLL_PROFILE = (Path(__file__).resolve().parent /
                              'stub_profile_native_dll.bin')
...
def _pe_rva_to_raw(buf, rva, size=1):
    """Map an RVA through the PE file's loader-visible section table.
    Unlike pefile's permissive mapping, this requires the bytes to be covered by
    SizeOfRawData; CLR metadata outside that span is exactly the corruption that
    previously made managed output report BadImageFormat."""
    pe = u32(buf, 0x3C)
    opt = u16(buf, pe + 20)
    nsec = u16(buf, pe + 6)
    shdr = u32(buf, pe + 84)
    if rva < shdr and rva + size <= len(buf):
        return rva
    st = pe + 24 + opt
    for i in range(nsec):
        s = st + i * 40
        ...
        rawptr = u32(buf, s + 20)
        delta = rva - va
        if 0 <= delta and delta + size <= rawsz:
            off = rawptr + delta
            if off + size <= len(buf):
                return off
    raise ValueError(f'RVA 0x{rva:X}+0x{size:X} is not loader-visible in packed PE')


def _materialize_managed_loader_view(out, T, tgt):
    """Put the target COR20 header and BSJB metadata in the raw mapped section.

    CrackProof's DLL runtime is a native/mixed-mode bootstrap, but Windows/.NET
    still reads the CLR directory and assembly metadata from the on-disk mapped
    image.  The compressed file-data layer is not enough for that early read.
    """
    if not tgt['clr_rva']:
        return
    pe = u32(out, 0x3C)
    ...
    if packed_clr_rva != tgt['clr_rva']:
        raise ValueError(f'managed target CLR RVA 0x{tgt["clr_rva"]:X} does not match '
                         f'profile CLR RVA 0x{packed_clr_rva:X}')
    if tgt['clr_rva'] + 0x48 > len(T):
        raise ValueError('target COR20 header is outside SizeOfImage')
    cor = bytes(T[tgt['clr_rva']:tgt['clr_rva'] + 0x48])
    md_rva = u32(cor, 8)
    md_size = u32(cor, 12)
    if not md_rva or md_size < 4 or md_rva + md_size > len(T):
        raise ValueError('target COR20 metadata range is invalid')
    if bytes(T[md_rva:md_rva + 4]) != b'BSJB':
        raise ValueError('target CLR metadata does not start with BSJB')
    cor_off = _pe_rva_to_raw(out, packed_clr_rva, len(cor))
    md_off = _pe_rva_to_raw(out, md_rva, md_size)
    out[cor_off:cor_off + len(cor)] = cor
    out[md_off:md_off + md_size] = T[md_rva:md_rva + md_size]


def _validate_loader_view(out, T, tgt):
    """Fail closed if a managed target became native or its metadata is hidden."""
    pe = u32(out, 0x3C)
    if bytes(out[pe:pe + 4]) != b'PE\x00\x00':
        raise ValueError('packed output lost its PE signature')
    out_is_dll = bool(u16(out, pe + 22) & 0x2000)
    if out_is_dll != tgt['is_dll']:
    ...
    md_rva = u32(out, cor_off + 8)
    md_size = u32(out, cor_off + 12)
    md_off = _pe_rva_to_raw(out, md_rva, md_size)
    if bytes(out[md_off:md_off + 4]) != b'BSJB':
        raise ValueError('packed CLR metadata is not loader-visible BSJB data')
    target_cor = tgt['clr_rva']
    target_md_rva = u32(T, target_cor + 8)
    target_md_size = u32(T, target_cor + 12)
    if md_size != target_md_size or bytes(out[md_off:md_off + md_size]) != bytes(T[target_md_rva:target_md_rva + target_md_size]):
        raise ValueError('packed loader-visible CLR metadata does not match target')
...
def pack_program(prof, S_fd, T, use_lz=False, do_imports=True, overlap=False):
    ...
    if tgt['clr_rva'] and not same_layout:
        raise ValueError('managed PE layout is incompatible with the selected CLR runtime '
                         'profile; refusing cross-layout output because it would be a '
                         'BadImageFormat/native-looking DLL')
    ...
    if tgt['clr_rva']:
        _materialize_managed_loader_view(out, T, tgt)
    _validate_loader_view(out, T, tgt)
...
def _rebuild_packed_header(out, S_fd, prof, tgt):
    ...
    for i in range(tgt['nsec']):
        ...
        # Keep the donor's loader-visible raw span.  A managed CrackProof DLL
        # intentionally maps most of .text from disk so CLR can read COR20 and
        # BSJB metadata before/while the native stub runs.  Shrinking this to a
        # single page makes the packed file look native and GetAssemblyName fails.
        donor = s_sectab + i * 40
        w32(out, dst + 16, u32(S_fd, donor + 16))
        w32(out, dst + 20, 0)  # PtrRaw is set by relocate_sections
...
def verify(out, T, prof, prog_end, aes_dir, tag='pack'):
    ...
    # Nyadecrypt). unpack_template leaves the shell's leftover bytes
...
def _target_range(buf, off, size, label):
    if off < 0 or size < 0 or off + size > len(buf):
        raise ValueError(f'{label} is outside target (off=0x{off:X}, size=0x{size:X})')


def load_target_image(path, layout='auto'):
    """Return a flat file-offset==RVA target image for the CrackProof file loop."""
    raw = bytearray(Path(path).read_bytes())
    if len(raw) < 0x40 or raw[:2] != b'MZ':
        raise ValueError('target is not a PE file (missing MZ header)')
    pe = u32(raw, 0x3C)
    _target_range(raw, pe, 24, 'PE header')
    ...
    _target_range(raw, pe + 24, opt, 'optional header')
    image_size = u32(raw, pe + 80)
    headers_size = u32(raw, pe + 84)
    if image_size < 0x1000 or headers_size == 0 or headers_size > image_size:
        raise ValueError(f'invalid PE image/header size: image=0x{image_size:X}, headers=0x{headers_size:X}')
    sectab = pe + 24 + opt
    _target_range(raw, sectab, nsec * 40, 'section table')
    sections = []
    for i in range(nsec):
        s = sectab + i * 40
        sections.append((u32(raw, s + 8), u32(raw, s + 12), u32(raw, s + 16),
                         u32(raw, s + 20), s))
    flat_layout = all(rawsz == 0 or rawptr == va for vs, va, rawsz, rawptr, s in sections)
    if layout == 'auto':
        layout = 'flat' if len(raw) >= image_size and flat_layout else 'file'
    if layout == 'flat':
        if len(raw) < image_size:
            raise ValueError(f'flat target is shorter than SizeOfImage: 0x{len(raw):X} < 0x{image_size:X}')
        del raw[image_size:]
        return raw
    if layout != 'file':
        raise ValueError(f'unknown input layout: {layout}')
    image = bytearray(image_size)
    image[:min(headers_size, len(raw))] = raw[:min(headers_size, len(raw))]
    for vs, va, rawsz, rawptr, sh in sections:
        if va >= image_size:
            continue
        if rawptr >= len(raw):
            continue
        count = min(rawsz, len(raw) - rawptr, image_size - va)
        image[va:va + count] = raw[rawptr:rawptr + count]
        w32(image, sh + 16, min(vs, image_size - va))
        w32(image, sh + 20, va)
    return image
...
def _do_pack(argv):
    ...
    ap.add_argument('--input-layout', choices=('auto', 'flat', 'file'), default='auto',
                    help='target layout: auto-detect, flat RVA image, or ordinary PE file')
    args = ap.parse_args(argv)
    aes_dir = Path(args.aes_dir) if args.aes_dir else None

    r.load_tables(aes_dir)
    tgt_path = Path(args.target)
    T = load_target_image(tgt_path, args.input_layout)
    tgt_info = parse_target(T)

    if args.profile:
        prof_path = Path(args.profile)
        profile_reason = 'explicit'
    elif tgt_info['clr_rva'] and tgt_info['is_dll']:
        prof_path = BUNDLED_DOTNET_DLL_PROFILE
        profile_reason = '.NET DLL (CLR directory present)'
    elif tgt_info['is_dll']:
        prof_path = BUNDLED_NATIVE_DLL_PROFILE
        profile_reason = 'native DLL'
    elif tgt_info['clr_rva']:
        raise ValueError('managed EXE needs a managed-EXE CrackProof runtime profile; '
                         'refusing to apply the native EXE profile')
    else:
        prof_path = BUNDLED_PROFILE
        profile_reason = 'native EXE'

    if not Path(prof_path).exists():
        print(f'profile not found for {profile_reason}: {prof_path}\n'
              f'(extract one with: python Nyaencrypt.py extract <packed_sample.exe> <profile>)')
        return 2
    ...
    print(f'[*] packing {tgt_path.name} into stub '
          f'(profile={Path(prof_path).name}, selected={profile_reason}, '
          f'info3=0x{prof.info[3]:X}) ...')
...
def main(argv=None):
    ...
    try:
        ...
        return 130
    except (OSError, ValueError, struct.error, pickle.PickleError, zlib.error) as exc:
